...
import { call, FORMATS, label, PLATFORMS, TONES, TOPICS, useQuery, BackendError, type Brief, type Project, type Settings } from '../lib/studio'
...
export function Create({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  return (
    <form onSubmit={submit} onKeyDown={(e) => (e.ctrlKey || e.metaKey) && e.key === 'Enter' && submit()}>
      ...
      <div className="grid grid-cols-[400px_minmax(0,1fr)] gap-12 max-[1180px]:grid-cols-[340px_minmax(0,1fr)]">
        <div className="grid content-start gap-7">
          ...
          <div className="grid gap-2 border-t border-line pt-6">
            <div><Button variant="primary" type="submit" disabled={!valid || saving} kbd="Ctrl Enter">{saving ? 'Saving' : 'Create project'}</Button></div>
            <p className="text-xs text-ink-3">Saves the brief as a draft project. AI generation is not connected yet.</p>
          </div>
        </div>
...
function BatchPreview({ brief }: { brief: Brief }) {
  ...
  return (
    <section aria-label="Batch preview" className="sticky top-0 self-start rounded-panel bg-panel p-6">
      <div className="flex items-baseline justify-between gap-4 pb-5">
        <h2 className="text-[13px] font-medium">{brief.topic.trim() || 'Untitled'}, {brief.tone}</h2>
        ...
        {frames.map((i) => (
          <div key={`${brief.format}-${i}`} className="anim-slot grid gap-1.5" style={{ '--i': i } as React.CSSProperties}>
            <div className={cx('grid gap-1.5', kinds.length === 2 && 'grid-cols-[9fr_7.2fr] items-end')}>
              {kinds.map((k) => (
                <div key={k} className={cx('rounded-[4px] border border-line-strong bg-raised/60', k === 'video' ? 'aspect-[9/16]' : 'aspect-[4/5]')} />
              ))}
            </div>
